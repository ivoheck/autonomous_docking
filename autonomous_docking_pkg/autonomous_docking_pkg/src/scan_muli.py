...
class QrScan(Node):
    ...
    def __init__(self):
        ...
        self.state = True
        self.bin_size = 3 #Anzahl der scans die zusammen genommen werden um genauigkeit zu erhöhen
        self.start_scan()
        ...
        if cam:
            while self.state:
                try:
                    state = self.bin_size
                    res = None
                    #Hier werden bis zu self.bin_size ergebnisse zusammen genommen sind diese scans alle erfolglos wird dieses ergebniss veröffentlicht
                    #dies soll verhindern das durch aussreißer die entscheidungsfindung gestört wird wenn etwa kein code gefunden wird obwohl eigentlch
                    #einer in dem Bild ist. Erst wenn das mehr mals hinter einander passiert wird dieses ergebniss verwendet
                    while state > 0:
                    ...
                    #Wenn die schleife kommplet durch läuft wurde kein qr gefunden
                    if state == 0:
